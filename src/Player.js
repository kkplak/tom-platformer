...
export default class Player extends GameObject {
  ...
  enableFlight() {
    this.canFly = true;
    document.querySelector(".fuel-label").classList.remove("hidden");
    document.querySelector(".fuel-bar").classList.remove("hidden");
    document.querySelector(".power-button").classList.remove("hidden");
    console.log("Flight ability enabled!");
  }
  ...
  fly(delta) {
    if (this.fuel > 0) {
      ...
      // this.soundManager.play("jetPack");
      this.y -= this.flightSpeed * delta;
  ...
  regenerateFuel(delta) {
    if (!this.isFlying && this.fuel < this.maxFuel) {
      this.fuel += this.fuelRegenRate * delta;
  ...
  jump() {
    ...
    console.log("Player is attempting to jump");
    console.log("this.soundManager:", this.soundManager);
    if (!this.soundManager) {
      console.error("soundManager is undefined in Player");
    } else {
  ...
  knockback() {
    ...
    this.soundManager.play("colide", { minInterval: 2000 });
    const knockbackForce = 2;
    this.velocityX = this.facing === 0 ? -knockbackForce : knockbackForce;

    this.velocityY = -2;

    setTimeout(() => {
      this.velocityX = 0;
    }, 3000); // check if its working
  }
  render(delta) {
    // animator
  ...
  update(delta) {
    ...
    const controller = this.game.controller;
    ...
    if (
      this.canFly &&
      (inputManager.isKeyDown(" ") || inputManager.isKeyDown("fly"))
    ) {
      this.fly(delta);
    } else {
      this.regenerateFuel(delta);
      this.stopFlying();
    }

    if (this.canFly && this.isFlying) {
      if (inputManager.isKeyDown(" ") || inputManager.isKeyDown("fly")) {
        this.y -= this.flightSpeed * delta;
      }
    } else {
    ...
    if (!this.animationName.includes("slide")) {
      ...
      this.velocityX =
        Math.sqrt(-0.07 * this.frictionValue + 3) * (this.facing == 0 ? -1 : 1);
      this.frictionValue++;
    }
    if (this.onGround) {
