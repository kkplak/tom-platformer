...
export default class Camera {
  ...
  setPositionCenterAndSize(centerX, centerY, width, height) {
    // Clamp width and height to map dimensions if necessary
    width = Math.min(width, this.game.levelWidth);
    height = Math.min(height, this.game.levelHeight);

    var cameraMax = [width, height];

    // Calculate new startX and startY
    let newStartX = centerX - cameraMax[0] / 2;
    let newStartY = centerY - cameraMax[1] / 2;

    // Clamp startX and startY to map boundaries
    newStartX = Math.max(
      this.minX || 0,
      Math.min(newStartX, (this.maxX || this.game.levelWidth) - cameraMax[0])
    );
    newStartY = Math.max(
      this.minY || 0,
      Math.min(newStartY, (this.maxY || this.game.levelHeight) - cameraMax[1])
    );

    this.startX = newStartX;
    this.startY = newStartY;
    this.endX = this.startX + cameraMax[0];
    this.endY = this.startY + cameraMax[1];
